...
```{r}
ggplot(data = admissions_tble) + 
  geom_bar(mapping = aes(x = difftime(dischtime, admittime, units = "weeks"))) +
  xlab("Length of hospital stay (weeks)") 
```

```{r}
ggplot(data = admissions_tble) + 
  geom_bar(mapping = aes(x = difftime(dischtime, admittime, units = "week"))) +
  scale_x_continuous(limits = c(0, max(difftime(
    admissions_tble$dischtime, admissions_tble$admittime, units = "week"))))
```


What might be unusual is that the bars are tiny and we can only see the bars on the bottom left- I tried with using the unit as days and weeks and both look similar. I'm assuming there are patients who stayed for a fairly long period (different in length among themselves as well), thus it is hard to observe these people directly on the histogram.
...
There are slightly more males than females. The mean anchor age for all patients is around 63.5 with a median of 65.
Male patients are generally younger than female patients. The mean anchor age for male is 62.5 with a median of 64, whereas the mean anchor age for female is 64.8 with a median of 67.
